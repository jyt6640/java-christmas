...
### 기능
- [X] 크레스마스 디데이 할인 기능
    - [X] 1일 ~ 25일 사이에 있을 시 적용
    - [X] 1000원으로 시작하여 25일까지 100원씩 오름
    - [X] 최대 혜택 금액은 3,400원
- [X] 평일 할인 기능 (1일 ~ 31일) 
    - [X] 평일인지 확인
    - [X] 평일에는 디저트 메뉴를 개당 2,023원 할인
- [X] 주말 할인 기능 (1일 ~ 31일)
    - [X] 주말인지 확인
    - [X] 주말에는 메인 메뉴를 개당 2,203원 할인
- [X] 특별 할인 기능 (1일 ~ 31일)
    - [X] 이벤트 달력에 별이 있는 날인지 확인
    - [X] 이벤트 달력에 별이 있으면 총 주문 금액에서 1,000원 할인
- [X] 증정 이벤트 기능 (1일 ~ 31일)
    - [X] 할인 전 총 주문 금액이 12만원 이상인지 확인
...
### 예외
- [ ] 방문 날짜가 1~31이 아닐 시 예외 발생 ([ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요.)
- [X] 방문 날짜가 숫자가 아닐 시 예외 발생 ([ERROR] 숫자만 입력해 주세요. 다시 입력해 주세요.)
- [X] 메뉴판에 없는 메뉴을 입력 시 예외 발생 ([ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.)
- [X] 메뉴의 개수가 1개 이상 입력하지 않을 시 예외 발생 ([ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.)
- [X] 메뉴 입력 형식이 다를 시 예외 발생 ([ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.)
- [ ] 중복 메뉴 입력 시 예외 발생 ([ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.)
- [ ] 음료만 주문 시 예외 발생 ([ERROR] 음료만 주문이 불가능합니다. 다시 입력해 주세요.)
- [ ] 메뉴가 20개 초과일 시 예외 발생 ([ERROR] 메뉴는 20개까지만 주문할 수 있습니다.)
...
- 이벤트 플래너가 주문 메뉴, 할인 전 총주문 금액, 증정 메뉴, 혜택 내역, 총혜택 금액, 할인 후 예상 결제 금액, 12월 이벤트 배지 내용을 보여주기를 기대합니다.
